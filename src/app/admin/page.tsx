...
// --- Helper Components ---
const DeleteConfirmationModal = ({ isOpen, onClose, onConfirm, loading, itemType }: { isOpen: boolean; onClose: () => void; onConfirm: () => void; loading: boolean; itemType: string; }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-xl w-full max-w-md p-6 text-center">
        ...
        <p className="text-gray-400 mb-8">Are you unequivocally certain you wish to permanently erase this {itemType}? This action cannot be undone.</p>
        <div className="flex justify-center gap-4">
          <button onClick={onClose} disabled={loading} className="px-6 py-2 rounded-lg bg-zinc-700 text-white font-semibold hover:bg-zinc-600 transition-colors duration-200 disabled:opacity-50">Cancel</button>
          <button onClick={onConfirm} disabled={loading} className="px-6 py-2 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center">
...
export default function AdminDashboard() {
  ...
  const handleAddCareer = async (e: FormEvent) => {
      ...
      finally { setIsAddingCareer(false); }
  };
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/admin/${itemToDelete.type}s/${itemToDelete.id}`, { method: "DELETE" });
      const data = await res.json();
      if (data.success) {
      ...
      } else { console.error(`Failed to delete ${itemToDelete.type}:`, data.error); }
    } catch (err) { console.error(err); } 
    finally {
      setIsDeleting(false);
      closeDeleteModal();
  ...
  return (
    <>
      <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={closeDeleteModal} onConfirm={confirmDelete} loading={isDeleting} itemType={itemToDelete?.type || ''} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-8">
                <div className="lg:col-span-2 space-y-12">
                    <section>
                        ...
                        {pendingApplications.length === 0 ? <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-8 text-center"><p className="text-gray-400">No pending applications.</p></div> : renderTable(pendingApplications, true)}
                    </section>
                    <section>
                        <div className="flex items-center gap-3 mb-6">
                ...
                <div className="lg:col-span-1 mt-12 lg:mt-0">
                    <section>
                        ...
                        <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 sticky top-8">
                            <form onSubmit={handleAddCareer} className="flex flex-col items-center gap-4 mb-6">
                                ...
                                <button type="submit" disabled={isAddingCareer} className="w-full flex-shrink-0 py-3 px-6 bg-orange-500 text-black font-bold rounded-xl hover:bg-orange-600 transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2"><PlusIcon/> Add Opening</button>
                            </form>
                            <div className="space-y-3 max-h-96 overflow-y-auto">
                                {careers.length > 0 ? careers.map(career => (
                                    <div key={career._id} className="flex items-center justify-between bg-zinc-900 p-4 rounded-lg">
                                        ...
                                        <button onClick={() => openDeleteModal(career._id, 'career')} className="p-2 rounded-lg text-red-500 hover:bg-red-500/10 transition-colors duration-200"><TrashIcon /></button>
                                    </div>
                                )) : <p className="text-center text-gray-500 py-4">No career openings found. Add one above.</p>}
                            </div>
                        </div>
                    </section>
                </div>
